...
function WRGF_ShortCode_Page( $Id ) {
    ...
    <style>
        #weblizar_<?php echo esc_attr($WRGF_Id); ?> .wrgf-header-label {
            color: <?php echo esc_attr($WL_Hover_Text_Color); ?> !important;
        }

        #weblizar_<?php echo esc_attr($WRGF_Id); ?> .wrgf-footer-label {
            color: <?php echo esc_attr($WL_Footer_Text_Color); ?> !important;
            font-size: 16px;
            margin-bottom: 5px;
            margin-top: 5px;
            text-align: center;
        ...
        #weblizar_<?php echo esc_attr($WRGF_Id); ?> .b-link-stroke .b-top-line {
            background: rgba(<?php echo esc_attr($HoverColorRGB); ?>, <?php if( $WL_Hover_Color_Opacity=="yes"){echo "0.5";} else{echo "1.0";} ?>);
        }

        #weblizar_<?php echo esc_attr($WRGF_Id); ?> .b-link-stroke .b-bottom-line {
            background: rgba(<?php echo esc_attr($HoverColorRGB); ?>, <?php if( $WL_Hover_Color_Opacity=="yes"){echo "0.5";} else{echo "1.0";} ?>);
        }

        #weblizar_<?php echo esc_attr($WRGF_Id); ?> .b-wrapper {
        ...
        <?php }?>
        #weblizar_<?php echo esc_attr($WRGF_Id); ?> a {
            border-bottom: none;
            overflow: hidden;
            float: left;
            margin-right: 0px;
    ...
    <div class="gal-container">
		<?php while ( $loop->have_posts() ) : $loop->the_post(); ?>
			...
			<?php if ( $WL_Show_Gallery_Title == "yes" ) { ?>
                <!-- gallery title -->
                <div style="font-weight: bolder; padding-bottom:10px; border-bottom:2px solid #cccccc; margin-bottom:20px; font-size:24px; font-family: <?php echo esc_attr($WL_Font_Style); ?>">
				...
				foreach ( $WRGF_AllPhotosDetails as $WRGF_SinglePhotoDetails ) {
                    ...
                    <div class="<?php echo esc_attr($WL_Gallery_Layout); ?> col-sm-6 wl-gallery">
                        <div class="b-link-<?php echo esc_attr($WL_Hover_Animation); ?> b-animate-go">
							...
							// swipe box
							?>
                            <a alt="<?php echo esc_attr($name); ?>" class="swipebox" href="<?php echo esc_url($url); ?>"
                               title="<?php echo esc_attr($name); ?>">
                                <img src="<?php echo esc_attr($Thummb_Url); ?>" class="gall-img-responsive"
                                     alt="<?php echo isset($slide_alt ) ? $slide_alt : '' ; ?>">
                                <div class="b-wrapper">
									<?php if ( $WL_Gallery_Layout == "col-md-12" || $WL_Gallery_Layout == "col-md-6" || $WL_Gallery_Layout == "col-md-4" || $WL_Gallery_Layout == "col-md-3" ) { ?>
										<?php if ( $WL_Show_Image_Label == "yes" ) { ?>
                                            <h2 class="b-from-left b-animate b-delay03 wrgf-header-label"><?php echo esc_html($name); ?> </h2>
										<?php } ?>
									<?php }
									?>
                                </div>
						...
						<?php if ( $WL_Show_Image_Label == "yes" ) { ?>
                            <h4 class="wrgf-footer-label"><?php echo esc_html($name); ?></h4>
						<?php } ?>
                    </div>
					<?php
				}
